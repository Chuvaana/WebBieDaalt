...
function createData(no, code, type, name, startCount,) {
...
export default function ProductList1() {
    ...
    useEffect(() => {
        axios.get("http://localhost:5000/api/user/getter")
            .then(res => {
    ...
    const rows = ringItems.map((item, index) =>
        createData(index + 1, item._id, item.name, item.phoneNumber, item.password)
    );
    ...
    return (
        <div className='workerList_frame'>
            <div className="header_medeell">
                <h1 className="baraa_garchig">Хэрэглэгчийн жагсаалт</h1>
