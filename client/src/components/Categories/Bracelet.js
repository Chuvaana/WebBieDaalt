...
const plainOptions = ["Алт", "Мөнгө"]; // Add your color options here
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        const ringItems = res.data.filter(
  ...
  const filterByPriceRange = () => {
    const filteredItems = initialRingItems.filter((item) => {
      ...
      return (
        price >= minPrice &&
        price <= maxPrice &&
        (checkedColors.length === 0 || checkedColors.includes(item.color))
            ...
            <div className="form-group">
              ...
              <Checkbox
                ...
                indeterminate={
                  ...
                  checkedColors.length < plainOptions.length
                }
                onChange={onCheckAllChange}
                checked={checkedColors.length === plainOptions.length}
              >
                Бүгд
              </Checkbox>
              <CheckboxGroup
